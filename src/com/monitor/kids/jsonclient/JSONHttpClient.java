...
public class JSONHttpClient {
    ...
    public boolean doRegistion(String account, String code) throws JSONClientException
    {
        try {
            ...
            jsonParams.put(JSONParams.APP_VERSION, KidsApplication.getVersion());
            jsonParams.put(JSONParams.DEVICE_IMSI, KidsApplication.getIMSI());
    ...
    private String getDeviceId() {
        String imei = KidsApplication.getIMEI();
        if (imei == null || imei.equals("0")) {
            String uid;
            if (KidsApplication.getConfiguration().getUUid() != null) {
                uid = KidsApplication.getConfiguration().getUUid();
            } else {
                uid = JSONParams.DEVICE_UUID + UUID.randomUUID().toString();
                KidsApplication.getConfiguration().setUUid(uid);
            }
            return uid;
        } else {
            return imei;
