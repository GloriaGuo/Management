...
public class KidsProvider extends ContentProvider {
	...
	/**
	 * Monitor For Kids authority for content URIs
	 */
	public static final String AUTHORITY = "com.parent.provider.kids";
	public static final String DATABASE_NAME = "KIDS";
	public static final int DATABASE_VERSION = 2;
	public static final String TAG = KidsApplication.getApplicationTag()
	        + "." + KidsProvider.class.getSimpleName();
	public static final String EXTERNAL_STORAGE_PATH = Environment.getExternalStorageDirectory() + "/" + ".kids";
	...
	private static class KidsDatabaseHelper extends SQLiteOpenHelper {
		...
		KidsDatabaseHelper(final Context context) {
...
//				Log.d(TAG, "getReadableDatabase, checking external storage: " + mount);
			...
			try {
				...
				final File target = new File(KidsProvider.EXTERNAL_STORAGE_PATH);
				if (!target.exists()) {
				    target.mkdirs();
				}
...
//				Log.d(TAG, "getWritableDatabase, checking external storage: " + mount);
			...
			try {
				...
				if (!target.exists()) {
                    ...
                    if (!rtn) {
    	                Log.e(TAG, "Couldn't mkdir: " + KidsProvider.EXTERNAL_STORAGE_PATH);
                    }   
				}
				db = SQLiteDatabase.openOrCreateDatabase(KidsProvider.getStoragePath() + "/" + DATABASE_NAME +".sqlite", null);
	...
	private KidsDatabaseHelper mOpenHelper;
...
//		if("internal".equals(
//				KidsApplication.getServiceConfiguration().getParameter("storage_type"))) {
//			// Use internal storage
//			return KidsApplication.getInternalPath();
//		} else {
...
//		}
	...
	public boolean onCreate() {
		try {
			mOpenHelper = new KidsDatabaseHelper(getContext());
	...
	public Cursor query(Uri uri, String[] projection, String selection,
        ...
        try {
            ...
            // arm flag indicating that the application cannot write the db
//            KidsApplication.readOnlyMode(true);
            return null;
        }
	}
...
//            tryRestoreWriteMode();
	...
	static {
		...
		sUriMatcher.addURI(KidsProvider.AUTHORITY, BrowserHistory.PATH, BrowserHistory.MATCHER);
        sUriMatcher.addURI(KidsProvider.AUTHORITY, BrowserBookmark.PATH, BrowserBookmark.MATCHER);
        sUriMatcher.addURI(KidsProvider.AUTHORITY, Gps.PATH, Gps.MATCHER);
        sUriMatcher.addURI(KidsProvider.AUTHORITY, AppsInstalled.PATH, AppsInstalled.MATCHER);
        sUriMatcher.addURI(KidsProvider.AUTHORITY, AppsUsed.PATH, AppsUsed.MATCHER);
		sUriMatcher.addURI(KidsProvider.AUTHORITY, RESET_PATH, MATCHER_RESET);
	}
}
